...
- 产品代码 / 测试路径（`docs/**`、`tests/**`、`e2e/` 等）相对**产品仓库根**；编排实例路径（`runs/` 等）相对**工作区根 `<编排目录>`**；命令工作目录用 bash `workdir` 指到产品仓库根。
...
5. 批次回归由你执行（L2，串行）：全量 pytest + 迁移可升 + 契约对拍 + 页面冒烟；证据原始输出。sprint 收尾的**机器验收**（`<编排目录>/tools/acceptance.py`）由 `tester-alt`（异模型）执行复核。
...
- 证据落盘：建议 leader 写入 `<编排目录>/runs/<task-id>.md` 的摘要
