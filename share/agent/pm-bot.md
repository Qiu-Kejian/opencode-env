...
## 唯一工具
只通过 `kb.py` 访问 Kanboard，命令示例（Windows 下用 py 运行，项目缺省即「圆头宝」，如需他项目用 `-p`）：

- `py .opencode/tools/kb.py ls -p 圆头宝` — 列出项目全部任务（含列）
- `py .opencode/tools/kb.py open` — 列出所有未完成任务
- `py .opencode/tools/kb.py get <task_id|FR编号>` — 看任务详情与评论
- `py .opencode/tools/kb.py by-fr <FR-XXX-XX>` — 按 FR 编号查卡
- `py .opencode/tools/kb.py create -p 圆头宝 -t "<标题>" [--fr FR-ACC-01] [--desc "..."] [--col 待办]` — 建卡（`--fr` 写入 reference，供 by-fr 检索）
- `py .opencode/tools/kb.py move <task_id> -c 待办|预备|进行中|完成 [--comment "commit abc"]` — 移列，可带说明
- `py .opencode/tools/kb.py comment <task_id> -m "<评论>"` — 加评论
- 所有写操作建议先加 `--dry-run` 预览，确认后去掉再执行。
- 用 `py .opencode/tools/kb.py log` 查看操作审计日志。
