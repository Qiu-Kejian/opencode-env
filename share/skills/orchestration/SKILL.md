...
> 本 skill 是**机制模板**；项目实例（sprint 文件、决策记录、验收器等）留在各项目仓库（默认 `orchestration/`，机制全文以该项目 `orchestration/README.md` 为准）。

## 1. 角色（agent 定义在 `.opencode/agent/`）

| 角色 | agent | mode | 职责 | 写权限 |
|---|---|---|---|---|
| leader | `leader` | primary | 拆卡、派发、仲裁、质量门、合并提交、写决策 | 只写 `orchestration/` 与机械性小修（≤10 行，须记录） |
...
- **模型分层**：实现与验收模型不同源（任务级用常规模型；sprint 验收复核用替代模型），降低同构盲区。
...
4. 并发上限 2–3；每任务一个 commit（`<type>(<scope>): <摘要> [T-xx]`），leader 统一提交，不 push。
...
- 批次门 = **机器验收**（项目 `orchestration/tools/acceptance.py`，退出码判定，报告 `runs/<S>-acceptance.md`）：静态门 + 分层测试 + 迁移升降（隔离库）+ `git status` 干净 + 敏感文件扫描 + 证据文件完整性；由 `tester-alt`（异模型）复核报告与 `[人工]` 项。
...
| `milestones/<M>.md` | 阶段验收包 + 人工结论 | leader / 主会话 |
| `tools/acceptance.py` | 批次门机器验收器 | 人工维护 |
| `templates/` | 任务卡 / sprint / milestone 模板 | 人工维护 |
...
3. 开场指令模板：`按 orchestration/README.md 执行 <目标>；输入：<任务书路径 或 FR/design 引用>；默认全自动，硬停项记入 decisions.md 并继续。`
...
- 状态事实源 = `sprints/<S>.md`；中断恢复：读 sprint 文件 → `runs/` → `git log`，按在途标记判定续跑，恢复动作留痕。
- 分支：一 sprint 一分支，batch 门全绿合并回 main（不 push）；失败分支保留。
- 收尾：验收清单分层标 `[机器]` / `[人工]`；机器全 PASS 且无 `[人工]` → 自动 accepted；阶段关闭时生成 milestone 验收包并**停止**下一阶段派发。

## 11. 会话派发与串行约束

- 同一工作树同一时刻最多 1 个未关闭 sprint；派发前检查 sprints 目录无进行中、看板无「进行中」卡。
